...
# We can use the NCBI SRA Toolkit to download the target FASTQ.gz data (https://www.ncbi.nlm.nih.gov/sra/?term=SRR17380406)
prefetch SRR17380406 --max-size u
fasterq-dump SRR17380406 --split-files -e 20 # Here we use 20 threads to speed up the downloading of this large-size file

# Alternatively, ffq can be used to download the SRR accession file
ffq SRR17380406
ffq --ftp SRR17380406 | jq -r '.[] | .url' | xargs curl -O
fasterq-dump SRR17380406 --split-files -e 20
