...
filter_alleles = function(geno_dat, threshold = .1) {
  ...
  for (i in 1:(ncol(geno_import) / 2)) {
    ...
    minor_allele <-
      names(geno_count[geno_count == min(geno_count)])[1] #grab first element incase we end up with a tie
...
xd_matrix_filtered = matrix(NA,
                            nrow = nrow(filtered_geno_dat),
                            ncol = ncol(filtered_geno_dat) / 2)
count = 1
for (i in seq(2, ncol(filtered_geno_dat), by = 2)) {
  xd_matrix_filtered[, count] <-
    xd_converter(filtered_geno_dat[, c(i - 1, i)])
  count = count + 1
}

p1 = ggplot(data.frame(y = xd_matrix_filtered[10, ], x = seq(1, ncol(xd_matrix_filtered)))) +
  geom_point(aes(x, y))
p1 = p1 + labs(
  x = "genome position",
  y = "Xd",
  title = paste(
    ...
    "genes remaining post filtering"
  )
)
